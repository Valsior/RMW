...
@section('content')
    <h2 class="text-center my-4">Modifier la catégorie {{$cate->categorie}}</h2>
    <div class="container">
        <div class="bg-secondary rounded-2 shadow">
            <form class="form-responsive w-75 mx-auto p-3" action="{{ route('categories.update', $cate->id ) }}" method="post">
                @method('PATCH')
                @csrf
                <div class="form-group">
                    <label for="categorie" class="form-text text-white font-weight-bold form-label">Nom de la catégorie</label>
                    <input type="text" name="categorie" class="form-control rounded-1" value="{{ $cate->categorie }}" required>
                </div>
                <div class="form-group">
                    <label for="description" class="form-text text-white font-weight-bold form-label">Description de la catégorie</label>
                    <textarea name="description" class="form-control rounded-1" id="" rows="5">{{ $cate->description }}</textarea>
                </div>
